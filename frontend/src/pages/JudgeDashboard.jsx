...
import { getCases, adjournCase, getAnalytics } from '../services/api';
...
export default function JudgeDashboard() {
  ...
  const handleDispose = (id) => setCases(cases.filter(c => c.id !== id));
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 20px' }}>
        ...
        <div style={card}>
          <div style={{ borderBottom: `2px solid ${C.border}`, paddingBottom: '15px', marginBottom: '25px' }}>
            <h3 style={{ color: C.primary, margin: 0, fontSize: '22px' }}>⚖️ AI-Prioritized Cause List</h3>
            <p style={{ color: '#64748b', fontSize: '13px', margin: '5px 0 0 0' }}>
              Ranked by JUSTIS Engine — P = (0.1·A) + (5·F·V·C) + 500Ω
            </p>
          </div>
          ...
          {cases.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px', background: '#f8fafc', borderRadius: '8px', border: '1px dashed #cbd5e1' }}>
              ...
              <h4 style={{ color: C.primary, marginTop: '15px' }}>Docket Cleared</h4>
            </div>
          ) : cases.map((c, i) => (
            <div key={c.id} style={{
              borderLeft: `6px solid ${c.omega ? '#f59e0b' : c.primary_case_nature === 'Criminal' ? '#ef4444' : '#3b82f6'}`,
              padding: '20px', margin: '16px 0', background: '#fff',
              borderRadius: '0 10px 10px 0', border: `1px solid ${C.border}`,
              ...
              {c.section_436a?.eligible && (
                <div style={{ background: '#fef2f2', border: '2px solid #ef4444', borderRadius: '8px', padding: '10px 14px', marginBottom: '12px' }}>
                  <strong style={{ color: '#b91c1c', fontSize: '13px' }}>⚠️ Section 436A Alert</strong>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                    ...
                    {c.omega && <span style={{ background: '#fef3c7', color: '#92400e', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '800' }}>⚠️ OMEGA</span>}
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '10px' }}>
                ...
                {c.under_sections && c.under_sections !== '—' && (
                  <span style={{ background: '#fef9c3', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', color: '#713f12', fontWeight: '600' }}>§ {c.under_sections}</span>
                )}
